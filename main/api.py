...
class StoreAPI(Resource):
    ...
    def post(self,AssignmentID):
        ...
        fetch = cursor.execute('SELECT * FROM assignment WHERE AssignmentID = ?',(AssignmentID)).fetchall()
        ...
        if fetch:
            ...
            cursor.execute('INSERT INTO assignments (TruckID, StoreID, SKUID, Entry Time, Exit Time, Loading Time) VALUES (?, ?, ?, ?, ?, ?)', 
                       (truck_id, store_id, sku_id, current_time, current_time, current_time))
            conn.commit()
            conn.close()
            return {'message': 'Assignment added successfully'}, 201
	@@ -401,18 +402,17 @@ def put(self):
        truck_id = data.get('truck_id')
        store_id = data.get('store_id')
        sku_id = data.get('sku_id')
        ...
        if fetch:
            cursor.execute('''UPDATE assignment SET TruckID=?, StoreID=?,
                           SKUID=?, EntryTime=?, ExitTime =?, LoadingTime=? WHERE AssignmentID=?''',(truck_id, store_id, sku_id, current_time , current_time, current_time,assignment_id))
            conn.commit()
            conn.close()
